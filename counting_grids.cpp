...
int main(){
    ...
    // Calculate (n0 - n1)^K and (n0 - n1)^(K-1)
    long long diff_pow_K = power(diff, K, MOD);
    long long diff_pow_K_minus_1 = (K == 1) ? 1 : power(diff, K - 1, MOD);
    
    // nm^(K-1)
    long long nm_pow_K_minus_1 = (K == 1) ? 1 : power(nm_mod, K - 1, MOD);
    ...
    // Total weight for color class c:
    // Wc = (NM)^K * K * (S0 * f_{K-1}(c) + S1 * f_{K-1}(1-c))
